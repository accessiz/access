...
const DialogContent = ({ className, children, showClose = true, ref, ...props }: DialogContentProps) => (
  <DialogPortal>
    <DialogOverlay />
    ...
    >
      {children}
      {showClose ? (
        <DialogPrimitive.Close className="absolute right-6 top-6 inline-flex h-12 w-12 md:h-10 md:w-10 items-center justify-center rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-2 focus:outline-ring focus:outline-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
          <X className="h-4 w-4" />
