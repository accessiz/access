    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon">
          ...
          {unseen > 0 && !showDotOnly && <span className="absolute -top-0.5 -right-0.5 inline-flex items-center justify-center rounded-full bg-destructive text-xs text-white px-1">{unseen}</span>}
        </Button>
      </PopoverTrigger>
        ...
        <ul className="space-y-2 max-h-64 overflow-auto">
          {activity.slice(0, 20).map(a => (
            <li key={a.id} className="text-sm">
              ...
              <div className="text-muted-foreground text-xs">{new Date(a.when).toLocaleString()}</div>
            </li>
          ))}
