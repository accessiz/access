...
import { createClient } from '@/lib/supabase/client'; // Solo para la URL pública
...
export default function ModelsPage() {
  ...
  useEffect(() => {
    // Obtenemos la URL pública de las imágenes una sola vez
    const supabase = createClient();
    const { data: urlData } = supabase.storage.from('models').getPublicUrl('');
    setPublicUrl(urlData.publicUrl);

    async function fetchCountries() {
        // Esta lógica puede quedarse aquí o moverse a la API, es pequeña
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      // Actualizamos el sortConfig desde la URL
      ...
      // Construimos la URL para nuestra API
  ...
  // El resto de las funciones (handleRowClick, createPageURL, etc.) se mantienen igual
