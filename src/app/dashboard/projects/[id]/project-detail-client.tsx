    ...
    <div className="flex items-center gap-4 p-2 hover:bg-muted/50 rounded-md">
        ...
        <Button size="icon" variant="ghost" onClick={onAction} disabled={isPending} className="h-8 w-8">
            {isPending ? <Loader2 className="animate-spin" /> : (
                actionType === 'add' ? <PlusCircle className="text-green-500" /> : <XCircle className="text-destructive" />
            )}
        </Button>
    </div>
);
            ...
            <header className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    <Button variant="outline" asChild>
                        <Link href={`/c/${project.id}`} target="_blank"><Eye /> Previsualizar</Link>
                    </Button>
                    <ShareProjectDialog project={project} onStatusChange={handleStatusChange}>
                        <Button><Share2 /> Compartir</Button>
                    </ShareProjectDialog>
                </div>
            </header>
                        ...
                        <ScrollArea className="h-96">
                            <div className="space-y-2 pr-4">
                                {availableModels.length > 0 ? availableModels.map(model => (
                                    <TalentRow key={model.id} model={model} onAction={() => handleAddModel(model.id)} isPending={isPending} actionType="add" />
                                )) : (
                        ...
                        <ScrollArea className="h-[28.5rem]">
                            <div className="space-y-2 pr-4">
                                {selectedModels.length > 0 ? selectedModels.map(model => (
                                    <TalentRow key={model.id} model={model} onAction={() => handleRemoveModel(model.id)} isPending={isPending} actionType="remove" />
                                )) : (
