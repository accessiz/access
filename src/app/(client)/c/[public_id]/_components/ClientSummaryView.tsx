    ...
    <div className="flex min-h-screen w-full flex-col bg-background text-foreground">
      <div className="w-full max-w-335 mx-auto px-6 md:px-0 flex flex-col flex-1">
        ...
        <main className="flex-1 py-12 md:py-20 flex flex-col items-center">
          ...
          {/* 2. GRID DE TALENTO APROBADO FRACCIONADO POR PAÍS */}
          {approved.length > 0 && (
            <div className="w-full mb-16 space-y-12">
              <h3 className="text-display border-b pb-4 uppercase tracking-tighter text-center md:text-left">Talento aprobado</h3>

              {/* Lógica de agrupación por país */}
              {Object.entries(
                approved.reduce((acc, model) => {
                  const country = model.birth_country || 'Sin Nacionalidad';
                  if (!acc[country]) acc[country] = [];
                  acc[country].push(model);
                  return acc;
